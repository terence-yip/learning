...
class message_encoder():
	...
	def encode_field(self, json_field):
		...
		if(field_format == "uint8"):
			return json_field["value"].to_bytes(1, self.byteorder, signed=False)
		if(field_format == "uint16"):
			return json_field["value"].to_bytes(2, self.byteorder, signed=False)
		if(field_format == "uint32"):
			return json_field["value"].to_bytes(4, self.byteorder, signed=False)
		if(field_format == "uint64"):
			return json_field["value"].to_bytes(8, self.byteorder, signed=False)
		if(field_format == "int8"):
			return json_field["value"].to_bytes(1, self.byteorder, signed=True)
		if(field_format == "int16"):
			return json_field["value"].to_bytes(2, self.byteorder, signed=True)
		if(field_format == "int32"):
			return json_field["value"].to_bytes(4, self.byteorder, signed=True)
		if(field_format == "int64"):
			return json_field["value"].to_bytes(8, self.byteorder, signed=True)
